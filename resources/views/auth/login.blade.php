        ...
        <div class="grid grid-cols-3 xl:grid-cols-5!">
            <div class="col-span-1 bg-carbon-950 h-dvh flex flex-col">
                ...
                <div class="mt-20 flex justify-center">
                    <div class="text-3xl font-rw-black text-white uppercase">{{ __('auth.login') }}</div>
                </div>
                <div class="mt-12 px-4">
                    ...
                    <form class="max-w-sm mx-auto" method="POST" action="{{ route('login.post') }}">
                        ...
                        <div class="flex items-start mb-5">
                            ...
                            <label for="remember" class="ms-2 text-sm font-rw-semibold text-white">{{ __('auth.remember_me') }}</label>
                        </div>
                        <div class="font-rw-semibold text-white text-sm mb-5">
                            {{ __('auth.no_account') }}
                            <a href="{{ route('register') }}" class="text-green-500 hover:underline">{{ __('auth.register') }}</a>.
                        </div>
                        <div class="flex items-center justify-end gap-x-4 mt-4">
                            ...
                            <button type="submit" class="font-rw-semibold text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">{{ __('auth.login') }}</button>
                        </div>
                    </form>
                </div>
            </div>
