<div>
    ...
    <x-containers.main>
        <div class="flex justify-between">
            <h1 class="text-xl font-semibold dark:text-white mb-4">{{ __('admin.titles.users_overview') }}</h1>
            ...
            <x-tables.table-striped>
                <x-slot name="headers">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-900 dark:text-white uppercase tracking-wider">{{ __('admin.labels.selected_language') }}</th>
                    ...
                    @forelse($users as $user)
                        <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-600 dark:even:bg-gray-700 border-b border-default">
                            ...
                            <td class="px-6 py-4 flex justify-end gap-x-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                @if ($user->accountLink)
                                    <x-tables.danger-action wire:click="unlinkAccount('{{ $user->uuid }}')">{{ __('admin.buttons.unlink') }}</x-tables.primary-action>
                                @endif
    ...
    <x-modals.modal wire:model="unlinkModal">
        <x-slot name="title">{{ __('admin.titles.users_modal_unlink') }}</x-slot>
        <x-slot name="content">
            <p>{{ __('admin.messages.users_modal_unlink_confirmation') }}</p>
        </x-slot>
        <x-slot name="footer">
    ...
    <x-modals.modal wire:model="deleteUserModal">
        <x-slot name="title">{{ __('admin.titles.users_modal_delete') }}</x-slot>
        <x-slot name="content">
            <p>{!! __('admin.messages.users_modal_delete_confirmation', ['name' => $selectedUser ? $selectedUser->name : '']) !!}</p>
        </x-slot>
        <x-slot name="footer">
            <button type="button" @click="$dispatch('close')" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 cursor-pointer">
                {{ __('general.cancel_button') }}
